...
```
#update backend.tf with right S3 bucket
terraform init
terraform plan
terraform apply
```
...
Steps to deploy on Vagrant(Local machine/Laptop):

```
cd vagrant-workspace/elkmaster
vagrant up
cd ../node1
vagrant up
cd ../../

# command to install elk master
ansible-playbook -i vagrant-inventory install/elk.yml 
# command to install elk node1
ansible-playbook -i vagrant-inventory install/elk-client.yml --extra-vars 'elk_server=192.168.33.45'
